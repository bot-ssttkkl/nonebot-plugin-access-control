...
def upgrade() -> None:
    with op.batch_alter_table('nonebot_plugin_access_control_rate_limit_rule', schema=None) as batch_op:
        ...
        batch_op.create_index('ix_ac_rate_limit_rule_subject_service',
    ...
    with op.batch_alter_table('nonebot_plugin_access_control_rate_limit_token', schema=None) as batch_op:
        ...
        batch_op.create_index(batch_op.f('ix_ac_rate_limit_token_rule_id'), ['rule_id'],
                              unique=False)
        batch_op.add_column(sa.Column("expire_time", sa.DateTime(), nullable=False, default=datetime.fromtimestamp(0)))
